...
from utils.graph import has_cycle, admg_to_pag, pag2cpag, compare_pag
...
def soft_ml4s_dcd_test(g_path: str, verbose:bool=False):
    ...
    est_dcd_g = pag2cpag(pag)
    if verbose: print("SA ML4S DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
...
def soft_order_ml4s_dcd_test(g_path: str, order:int, verbose:bool=False):
    ...
    pag = admg_to_pag(best_G)
    est_dcd_g = pag2cpag(pag)
    if verbose: print("SA Order ML4S DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
    return compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges())
...
if __name__ == "__main__":
    ...
    # for idx in [4]:
        ...
        rlt = soft_order_ml4s_dcd_test(g_p, 2, True)
        rlt["g"] = idx
        perf.append(rlt)
    import pandas as pd
    df = pd.DataFrame(perf)
    df.to_csv("sa_order_ml4s_1day_large.csv", index=False)