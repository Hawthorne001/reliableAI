...
import copy, sys, os, dill, time

sys.path.append("./")
from utils.fci import fci
from utils.ricf import bic
from utils.graph import admg_to_pag, pag2cpag, compare_pag, has_cycle, pprint_pag
from learn.gen_feature import Dataset
from learn.fine_tune import end2end_espi_sachs
...
class Discovery:
    ...
    def _create_bounds(self, tiers, unconfounded_vars, var_names, skeleton=None):
        """
        ...
        for i in range(len(var_names)):
            for j in range(len(var_names)):
                ...
                elif skeleton is not None and skeleton[i,j] == 0:
                    bounds_directed_edges.append((0, 0))
                ...
                if i <= j or (skeleton is not None and skeleton[i,j] == 0):
    ...
    def _compute_hard_skeleton_penalty(self, skeleton):
        penalty = np.concatenate((skeleton.flatten(), skeleton.flatten()))
        penalty[penalty == 0] = 0.1
        return penalty
    ...
    def _discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
                       ...
                       ricf_increment=1, ricf_tol=1e-4, verbose=False, skeleton=None, has_time_constraint=False):
        """
        ...
        bounds = self._create_bounds(tiers, unconfounded_vars, data.columns, skeleton)
        ...
        for num_iter in range(max_iter):
            if time.time() - start >= TIME_LIMIT and has_time_constraint: break
            ...
            print(num_iter, h, convergence)
            if h <= h_tol or rho >= rho_max:
                break

        final_W1, final_W2 = W1_hat.copy(), W2_hat + Wii_hat
        for w_thres in np.linspace(w_threshold, 1, 1000):
            final_W1[np.abs(final_W1) < w_thres] = 0
            final_W2[np.abs(final_W2) < w_thres] = 0
            if not has_cycle(final_W1):
                return self.get_graph(final_W1, final_W2, data.columns, w_thres), convergence
            else:
                print(f"[Warning]: cyclic; threshold:", w_thres)
        print(f"[Warning]: cyclic")
        return self.get_graph(np.zeros_like(final_W1), np.zeros_like(final_W2), data.columns, w_threshold), convergence
    ...
    def discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
                      ...
                      ricf_increment=1, ricf_tol=1e-4, verbose=False, detailed_output=False, skeleton=None, time_constraint=False, gt_pag=None):
        """
        ...
        global TIME_LIMIT
        TIME_LIMIT = 1 * 24 * 3600 / 32 / num_restarts
        ...
        for i in range(num_restarts):
            ...
            G, convergence = self._discover_admg(data, admg_class, tiers, unconfounded_vars, max_iter,
                                                 ...
                                                 ricf_tol, detailed_output, skeleton, time_constraint)
            if gt_pag: 
                pag = admg_to_pag(G)
                est_dcd_g = pag2cpag(pag)
                print("iter:",i,compare_pag(est_dcd_g.get_graph_edges(), gt_pag.get_graph_edges()))
            curr_bic = bic(data, G)
            if verbose:
...
def dcd_sachs():
    global TIME_LIMIT
    # TIME_LIMIT = 0xffffffffffffff
    # TIME_LIMIT = TIME_LIMIT - finetune_time/32
    learn = Discovery(lamda=0.05)
    data = pd.read_csv("data/sachs/sachs.csv")
    best_G = learn.discover_admg(data, admg_class="ancestral", verbose=True, num_restarts=5)
    pag = admg_to_pag(best_G)
    pprint_pag(pag)
    with open("data/sachs/dcd.pkl", "wb") as f:
        dill.dump(pag, f)

def original_dcd_test(g_path: str, verbose:bool=False):
    ...
    best_G = learn.discover_admg(data, admg_class="ancestral", verbose=False, num_restarts=1, time_constraint=True, gt_pag=cpag)
    pag = admg_to_pag(best_G)
    est_dcd_g = pag2cpag(pag)
    if verbose: print("Original DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
...
if __name__ == "__main__":
    ...
    #     rlt = original_dcd_test(g_p, True)
    #     rlt["g"] = idx
    #     perf.append(rlt)
    # import pandas as pd
    # df = pd.DataFrame(perf)
    # df.to_csv("original_dcd_1day_large.csv", index=False)
