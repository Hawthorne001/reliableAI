...
class Discovery:
    ...
    def _discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
        ...
        def penalized_gradient(X, *args):
            ...
            mask = (skl_penalty > np.random.random(skl_penalty.shape)).astype(float)
            mask[mask==0] = 0.1
            return mask * jac_mat
        start = time.time()
            ...
            while rho < rho_max:
                ...
                while ricf_iter < ricf_max_iters:
                    ...
                    sol = sopt.minimize(self.primal_loss, current_estimates,
                                        ...
                                        method='L-BFGS-B',
                                        options={'disp': False}, bounds=bounds, jac=penalized_gradient if num_iter < warm_up_iters else gradient)
...
def soft_ml4s_dcd_test(g_path: str, verbose:bool=False):
    ...
    if verbose: print("Stochastic ML4S DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
...
def soft_order_ml4s_dcd_test(g_path: str, order:int, verbose:bool=False):
    ...
    pag = admg_to_pag(best_G)
    est_dcd_g = pag2cpag(pag)
    if verbose: print("Stochastic Order ML4S DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
    return compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges())
...
if __name__ == "__main__":
    ...
    for idx in list(range(10)):
        np.random.seed(42)
        g_p = f"data/test/medium_{idx}"
        # print(soft_order_ml4s_dcd_test(g_p, 2, True))
        rlt = soft_ml4s_dcd_test(g_p, True)
        rlt["g"] = idx
        perf.append(rlt)
    import pandas as pd
    df = pd.DataFrame(perf)
    df.to_csv("stochastic_ml4s_dcd_medium.csv", index=False)