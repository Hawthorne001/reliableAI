...
from utils.graph import has_cycle, admg_to_pag, pag2cpag, compare_pag, pprint_pag
from learn.fine_tune import end2end_espi, end2end_espi_sachs
from learn.gen_feature import Dataset

TIME_LIMIT = 1 * 24 * 3600 / 32
...
class Discovery:
    ...
    def _discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
        ...
        skl_penalty = np.concatenate((skeleton.flatten(), skeleton.flatten()))
        ...
        def penalized_gradient(X, *args):
            ...
            directional_mask = (jac_mat * X > 0).astype(float)
            skeleton_mask = (np.power(skl_penalty, 1/(num_iter+2)) > np.random.random(skl_penalty.shape)).astype(float)
            final_mask = ((directional_mask != 0) | (skeleton_mask != 0)).astype(float)
            return final_mask * jac_mat
        
        start = time.time()
        # iterate till convergence or max iterations
        for num_iter in range(max_iter):
            if time.time() - start >= TIME_LIMIT: break
            else: print("remain time",TIME_LIMIT - (time.time() - start))
    ...
    def discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
                      ...
                      ricf_increment=1, ricf_tol=1e-4, verbose=False, detailed_output=False, skeleton=None, skl_threshold:float=-1, gt_pag=None):
        """
        ...
        for i in range(num_restarts):
            ...
            G, convergence = self._discover_admg(data, admg_class, tiers, unconfounded_vars, max_iter,
                                                 ...
                                                 ricf_tol, detailed_output, skeleton, skl_threshold)
            if gt_pag: 
                pag = admg_to_pag(G)
...
def espi_dcd_sachs():
    ...
    best_G = learn.discover_admg(data, admg_class="ancestral", verbose=True, skeleton=skl, num_restarts=5, skl_threshold=0.001)
    pag = admg_to_pag(best_G)
    pprint_pag(pag)
    with open("data/sachs/espi.pkl", "wb") as f:
        dill.dump(pag, f)
    

def espi_dcd_test(g_path: str, verbose:bool=False):
    dataset = Dataset(g_path)
    with open(os.path.join(g_path, "cpag.pkl"), "rb") as f:
        cpag : GeneralGraph= dill.load(f) 
    skl = end2end_espi(g_path)
    finetune_time = np.loadtxt(os.path.join(g_path,"vicinal_time.txt"))
    global TIME_LIMIT
    # TIME_LIMIT = TIME_LIMIT - finetune_time/32
    learn = Discovery(lamda=0.05)
    data = pd.DataFrame(dataset.X, columns=[f"X{i+1}" for i in range(dataset.D.shape[0])])
    best_G = learn.discover_admg(data, admg_class="ancestral", verbose=False, skeleton=skl, num_restarts=1, skl_threshold=0.001, gt_pag=cpag)
    pag = admg_to_pag(best_G)
    est_dcd_g = pag2cpag(pag)
    if verbose: print("SA ESPI DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
...
if __name__ == "__main__":
    ...
    #     rlt = espi_dcd_test(g_p, True)
    #     rlt["g"] = idx
    #     perf.append(rlt)
    # import pandas as pd
    # df = pd.DataFrame(perf)
    # df.to_csv("espi_large.csv", index=False)
