...
from learn.ml4s import end2end_ml4s_test
...
class Discovery:
    ...
    def _compute_skeleton_penalty_from_hard(self, skeleton):
        penalty = np.concatenate((skeleton.flatten(), skeleton.flatten()))
        penalty[penalty == 0] = 0.1
        return penalty
    
    def _compute_skeleton_penalty_from_soft(self, skeleton):
        penalty = np.concatenate((skeleton.flatten(), skeleton.flatten()))
        return penalty
    ...
    def _discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
                       ...
                       ricf_increment=1, ricf_tol=1e-4, verbose=False, skeleton=None):
        """
        ...
        if skeleton is not None:
            skl_penalty = self._compute_skeleton_penalty_from_hard(skeleton)
            skl_factor = copy.copy(skeleton)
            skl_factor[skl_factor == 0] = 0.1
        else:
            skl_penalty = np.concatenate((np.ones((d, d)).flatten(), np.ones((d, d)).flatten()))
            skl_factor = np.ones((d, d))
    ...
    def discover_admg(self, data, admg_class, tiers=None, unconfounded_vars=[], max_iter=100,
                      ...
                      ricf_increment=1, ricf_tol=1e-4, verbose=False, detailed_output=False, skeleton=None):
        """
        ...
        for i in range(num_restarts):
            ...
            G, convergence = self._discover_admg(data, admg_class, tiers, unconfounded_vars, max_iter,
                                                 ...
                                                 ricf_tol, detailed_output, skeleton)
            curr_bic = bic(data, G)
            if verbose:
...
def soft_fci_dcd_test(g_path: str, verbose:bool=False):
    ...
    fci_g, _ = fci(dataset.X, depth=1)
    if verbose:
        print("FCI",compare_pag(fci_g.get_graph_edges(), cpag.get_graph_edges()))
    skl = np.zeros_like(dataset.D)
    for ix, iy in np.ndindex(skl.shape):
        if fci_g.graph[ix, iy] != 0:
            skl[ix, iy] = 1
            skl[iy, ix] = 1
    learn = Discovery(lamda=0.05)
    data = pd.DataFrame(dataset.X, columns=[f"X{i+1}" for i in range(dataset.D.shape[0])])
    best_G = learn.discover_admg(data, admg_class="ancestral", verbose=False, skeleton=skl, num_restarts=1)
    pag = admg_to_pag(best_G)
    est_dcd_g = pag2cpag(pag)
    if verbose: print("Soft DCD", g_path, compare_pag(est_dcd_g.get_graph_edges(), cpag.get_graph_edges()))
...
if __name__ == "__main__":
    ...
    print(soft_fci_dcd_test(g_p, True))